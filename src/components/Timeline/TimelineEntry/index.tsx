    ...
    public render() {
        return (
            <div className={`flex items-center white`}>
                <span className={`mr2 tr normal timestamp ${this.state.beginAnimation ? 'fade-in-left' : 'hide'}`}>{this.props.timestamp}</span>
                <div className="bubble">
                    <TimelineBubble icon={this.props.icon} />
                </div>
                <div className={this.state.beginAnimation ? 'fade-in-right' : 'hide'}>
                    {this.props.children}
                </div>
            </div>
        );
